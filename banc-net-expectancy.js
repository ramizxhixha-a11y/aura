...
const TOK_10F = '20260917b';   // [1b-a · 14/09/2026] 10f relivré (sorties bot hors résolution) ; avant : 20260912c (PHASE 1), 20260906g (P7)
...
T('10f : version ' + TOK_10F + ', ≤ 500 lignes de code hors commentaires ? non — 715 lignes totales (module hérité, non redécoupé ici ; [P7] +18 ; [PHASE 1] +18 ; [1b-a] +40), 0 lecture de totalPnlPct / totalPnlUsd / _recentNet / _learned hors commentaires', () => {
  assert.ok(F.startsWith('// ▓▓▓ VERSION ' + TOK_10F + ' ▓▓▓'));
  assert.strictEqual(F.split('\n').length, 715);   // [1b-a hotfix b] 659 → 693 : +1 en-tête, +_closeCompleted, gardes dans les deux balayages   // [1b-a] 619 → 659 : +1 en-tête, +6 cibles _tpPct/_slPct (−11 breakeven/tpHit/slHit), +44 _botExitSweep ; [PHASE 1] 601 → 619
