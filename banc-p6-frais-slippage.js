...
T('09c : version 20260906e, anti-flood _costVetoLogTs déclaré une fois', () => { assert.ok(c9.startsWith('// ▓▓▓ VERSION 20260922b ▓▓▓')); assert.strictEqual(c9.split('const _costVetoLogTs = {};').length - 1, 1); });
...
T('09c : diff limité au bloc P6 (aucune autre fonction touchée : 1017 lignes, 984 identiques à la version 20260906d)', () => { assert.strictEqual(c9.split('\n').length, 1046); /* [PLAFOND DE SENS 21/09] +bloc plafond de sens, +1 ligne d'en-tête, +1 déclaration */ assert.strictEqual(c9.split('_betaGateForOpen(pair, side)').length - 1, 1); });
