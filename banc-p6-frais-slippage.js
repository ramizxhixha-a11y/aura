...
T('HTML : 77 ressources au token 20260906e, aucune au token précédent, DOC_V = 20260906e', () => { assert.strictEqual((html.match(/\?v=20260906e/g) || []).length, 77); assert.ok(!html.includes('20260906d')); assert.ok(html.includes("var DOC_V = '20260906e';")); });
...
T('10f (590 l.) non touché : aucune référence _costGate / 10e6 ; module 10e6 = 98 lignes ≤ 500', () => { const f = fs.readFileSync('js/10f-resolveur-cycle.js', 'utf8'); assert.strictEqual(f.split('\n').length, 591); assert.ok(!f.includes('_costGate')); assert.strictEqual(fs.readFileSync('js/10e6-frais-slippage.js', 'utf8').split('\n').length, 98); });
T('10e6 : node --check ; 5 exports window ; aucun s.replace / S.portfolio / botAutoMode / tradingMode écrit', () => { const m = fs.readFileSync('js/10e6-frais-slippage.js', 'utf8'); new vm.Script(m); assert.strictEqual((m.match(/^window\._\w+ = _\w+;$/gm) || []).length, 5); assert.ok(!/S\.(portfolio|botAutoMode|tradingMode)\s*=/.test(m)); });
