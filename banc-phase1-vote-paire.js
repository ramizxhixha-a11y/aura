...
T('en-têtes : 03/12 « [PHASE 1 · 12/09/2026] VERSION 20260912c », 02/08 relivrés par 1b-a « [1b-a · 14/09/2026] VERSION 20260914a », 10f « ▓▓▓ VERSION 20260917b ▓▓▓ »', () => {
  ...
  assert.ok(s03.startsWith('// [MACRO RÉEL · 26/09/2026] VERSION 20260926b') && s03.split('\n').slice(0, 15).some(l => l.startsWith('// [PHASE 1 · 12/09/2026] VERSION 20260912c')), F03);   // [FITNESS GLISSANTE] 03 relivré, en-tête PHASE 1 conservé dans les 5 premières lignes
  assert.ok(s08.startsWith('// [MACRO RÉEL · 26/09/2026] VERSION 20260926b') && s08.split('\n').slice(0, 8).some(l => l.startsWith('// [1b-a · 14/09/2026] VERSION 20260914a')), F08);   // [1b-b] 08 relivré, en-tête 1b-a en 2e ligne
  assert.ok(s02.startsWith('// [STOP CÔTÉ EXCHANGE SIMULÉ · 26/09/2026] VERSION 20260926a') && s02.split('\n').slice(0, 14).some(l => l.startsWith('// [1b-a · 14/09/2026] VERSION 20260914a')), F02);   // [SONDE RÉSEAU] 02 relivré, en-tête 1b-a en 2e ligne
  assert.ok(s10f.startsWith('// ▓▓▓ VERSION 20260926a ▓▓▓'));   // 10f livré au hotfix 1b-a (b), non retouché depuis
});
