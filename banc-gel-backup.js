...
(async () => {
  ...
  /* ───── statique ───── */
  await T('syntaxe : 03, 04, 09b3 compilent + en-têtes VERSION ' + VER03 + ' (03 et 04 : MASQUE CORRIGÉ 20260926p)', () => {
    for (const f of [F03, F04, F9B3]) { new vm.Script(src(f), { filename: f }); assert.ok(src(f).startsWith(f === F03 ? HEAD03 : f === F04 ? HEAD04 : HEAD), f + ' : en-tête'); if (f === F04) assert.ok(src(f).split('\n')[1].startsWith(HEAD), '04 : GEL BOOT en 2e ligne'); }
  });
