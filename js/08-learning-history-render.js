...
function resolvePairCycle(pair, ps) {
  ...
  if (dataAge > stalenessThreshold) {
    // Données obsolètes → pause cette paire
    if (!S.realKillSwitch) S.realKillSwitch = {};
    S.realKillSwitch[pair] = { paused:true, lossStreak:(ks?ks.lossStreak:0), reason:'Données obsolètes' };
    if (typeof showToast === 'function') showToast('⚠ ' + pair + ' · données obsolètes · pause auto', 4000, 'warn');
