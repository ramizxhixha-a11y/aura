...
// [REDÉMARRAGE · 26/09/2026] VERSION 20260926n · applySnap relit les délais de l'évolution, de la revigoration et du rêve (horodatage inconnu ou du futur : le délai part du chargement) ; _abstMigrated
// [MÉRITE DE L'ÉVOLUEUR · 26/09/2026] VERSION 20260926k · applySnap relit evoTrials, evoMerit, _metaMeritMigrated
// [MÉRITE DES BOTS · 26/09/2026] VERSION 20260926j · applySnap relit botMerit, _botPredictions, _botMeritMigrated
...
// [MÉMOIRE DE LA BLACKLIST · 22/09/2026] VERSION 20260922c · applySnap relit _lossStreaks
// [PLAFONDS APPRIS · 22/09/2026] VERSION 20260922b · applySnap relit capRules et recalcule les plafonds sur la mémoire
// [MÉMOIRE DES CHEMINS · 22/09/2026] VERSION 20260922a · applySnap relit horizonRules et recalcule les règles sur la mémoire
// [JOURNAL DES ÉVÉNEMENTS · 20/09/2026] VERSION 20260920b · applySnap relit eventLog / eventStats et réinstalle le relais
// [ATTRIBUTION PAR SOURCE · 17/09/2026] VERSION 20260917f · applySnap relit attribution
// [GÉNOME DE PAIRE · 17/09/2026] VERSION 20260917e · applySnap relit pairGenome / pairGenomeHistory / _pairGenomeDay
...
async function loadState() {
  ...
  try {
    ...
    if (snap.genome && typeof snap.genome === 'object')               S.genome        = snap.genome;          // [GÉNOME · 16/09/2026]
    if (snap.genomeHistory && typeof snap.genomeHistory === 'object') S.genomeHistory = snap.genomeHistory;   // [GÉNOME · 16/09/2026]
    if (snap.pairGenome && typeof snap.pairGenome === 'object')               S.pairGenome        = snap.pairGenome;          // [GÉNOME DE PAIRE · 17/09/2026]
    if (snap.pairGenomeHistory && typeof snap.pairGenomeHistory === 'object') S.pairGenomeHistory = snap.pairGenomeHistory;
    if (snap._pairGenomeDay && typeof snap._pairGenomeDay === 'object')       S._pairGenomeDay    = snap._pairGenomeDay;
    if (snap.attribution && typeof snap.attribution === 'object')             S.attribution       = snap.attribution;       // [ATTRIBUTION PAR SOURCE · 17/09/2026]
    if (snap.horizonRules && typeof snap.horizonRules === 'object')           S.horizonRules      = snap.horizonRules;      // [MÉMOIRE DES CHEMINS · 22/09/2026]
    if (snap.capRules && typeof snap.capRules === 'object')                   S.capRules          = snap.capRules;          // [PLAFONDS APPRIS · 22/09/2026]
    if (snap._gbpToBnbDone) S._gbpToBnbDone = true;
    if (isFinite(snap._realJudgments)) S._realJudgments = Number(snap._realJudgments) || 0;   // [COMPTEURS RÉGLAGES · 24/09/2026]
    ...
    if (snap.botMerit && typeof snap.botMerit === 'object')                   S.botMerit          = snap.botMerit;          // [MÉRITE DES BOTS · 26/09/2026]
    if (Array.isArray(snap._botPredictions))                                  S._botPredictions   = snap._botPredictions;
    if (snap._botMeritMigrated)                                               S._botMeritMigrated = true;
    if (snap.evoTrials && typeof snap.evoTrials === 'object')                 S.evoTrials         = snap.evoTrials;         // [MÉRITE DE L'ÉVOLUEUR · 26/09/2026]
    if (snap.evoMerit && typeof snap.evoMerit === 'object')                   S.evoMerit          = snap.evoMerit;
    if (snap._metaMeritMigrated)                                              S._metaMeritMigrated = true;
    ['_lastEvolutionAt', '_lastDreamAt'].forEach(function (k) {   // [REDÉMARRAGE · 26/09/2026] · [ÉVOLUTION SEULE · 26/09/2026] plus de délai de revigoration (retirée)
      var v = Number(snap[k]);
      if (v > 0 && v <= Date.now() + 60000) S[k] = v;
      // Heure inconnue (snapshot antérieur au 26/09 n) ou du futur (horloge reculée) : le délai part du chargement — sinon les
      // deux partaient à CHAQUE rechargement (capture Rams 26/09 22:09). Zéro enregistré = jamais arrivé : rien n'est retenu.
      else if (!(k in snap) || v > 0) S[k] = Date.now();
    });
    if (snap._abstMigrated)                                                   S._abstMigrated = true;   // [ABSTENTION · 26/09/2026]
    ...
    try { if (typeof _capRefresh === 'function') _capRefresh(); } catch(e) {}                                         // [PLAFONDS APPRIS · 22/09/2026] idem
  ...
  try {
    ...
    if (Array.isArray(snap.eventLog))               S.eventLog   = snap.eventLog.slice(-400);   // [JOURNAL DES ÉVÉNEMENTS · 20/09/2026]
    if (snap.eventStats && typeof snap.eventStats === 'object') S.eventStats = snap.eventStats;
    try { if (typeof _installChainTap === 'function') _installChainTap(); } catch(e) {}         // le tableau vient d'être remplacé
