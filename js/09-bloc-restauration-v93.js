...
function buildSnapshot() {
  const snap = {
    ...
    // v117 FIX BLOAT · le snapshot pré-réel n'a de sens QUE pendant le mode réel
    // (c'est sa cible de rollback). Hors mode réel il ne sert à rien : on ne le
    // persiste plus → fini les ~858 Ko de cadavre dans chaque backup en mode sim.
    ...
    // v117 FIX BLOAT · idem pour le jumeau paperReal : persisté uniquement en mode paperReal
...
function openDiagnostic() {
  ...
  // v117 FIX FAUSSE ALERTE · en mode sim ("Auto-apprentissage") les prix sont
  // SIMULÉS : aucun fetch HTTP, aucun WebSocket. "Dernier fetch" et "Paires
  // figées (STALE)" n'ont donc aucun sens en sim et affichaient un faux rouge
  // alarmant. En simulation rien n'est figé (bougies régénérées chaque cycle).
  ...
  // En sim : staleCount reste 0 — rien n'est réellement figé.
  ...
  // v117 FIX FAUSSE ALERTE · en sim, la "source prix réelle" n'a pas de sens
