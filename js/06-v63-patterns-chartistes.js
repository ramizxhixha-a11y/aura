...
function _abExecute(mode) {
  ...
  try {
    ...
    // ════════════════════════════════════════════════════════════
    // v117 DIAG TEMPORAIRE · le toast défile trop vite pour être lu.
    // Cet alert() (UNE SEULE FOIS par session) permet de LIRE et
    // SCREENSHOTER l'erreur exacte. → à retirer une fois la cause connue.
    // ════════════════════════════════════════════════════════════
    try {
      if (!window._abDiagShown) {
        window._abDiagShown = true;
        var _d = '🩺 BACKUP ÉCHOUÉ — DIAGNOSTIC\n\n';
        ...
        _d += '\nlocalStorage total : ' + Math.round(_tot/1024) + ' Ko\n';
        _d += 'Plus grosses cles :\n';
        for (var _m = 0; _m < Math.min(6, _all.length); _m++) {
