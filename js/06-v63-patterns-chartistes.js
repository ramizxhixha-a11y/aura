...
function _abExecute(mode) {
  ...
  try {
    ...
    // ════════════════════════════════════════════════════════════
    // v117 DIAG TEMPORAIRE · panneau PERSISTANT (ne se ferme pas tout
    // seul) avec bouton COPIER — pour récupérer l'erreur exacte.
    // → à retirer une fois la vraie cause connue.
    // ════════════════════════════════════════════════════════════
    try {
      if (!window._abDiagShown && !document.getElementById('_abDiagOverlay')) {
        window._abDiagShown = true;
        ...
        // panneau persistant injecté dans la page (ne disparait pas tout seul)
        var _ov = document.createElement('div');
        _ov.id = '_abDiagOverlay';
        _ov.style.cssText = 'position:fixed;inset:0;z-index:2147483647;background:rgba(0,0,0,.94);display:flex;flex-direction:column;padding:16px;box-sizing:border-box;font-family:monospace';
