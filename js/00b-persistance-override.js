...
(function() {
  ...
  async function _readIDB() {
    ...
    return await new Promise((resolve) => {
      try {
        ...
        // Essai 1 : lire avec la clé connue (SAVE_KEY)
        ...
        req.onsuccess = () => {
          ...
          // Essai 2 : si rien à la clé connue, lire tout le store
          // (au cas où l'ancien code aurait utilisé un keyPath différent)
          const reqAll = store.getAll();
          reqAll.onsuccess = () => {
            const arr = reqAll.result || [];
            if (arr.length === 0) { resolve(null); return; }
            // Prendre le plus récent par savedAt
            arr.sort((a, b) => {
  ...
  async function _writeIDB(snap) {
    ...
    // Vérifier que le store existe (sinon transaction échoue silencieusement)
    if (!db.objectStoreNames || !db.objectStoreNames.contains(STORE_STATE)) {
    ...
    return await new Promise((resolve) => {
      ...
      let req;
      try {
        // IMPORTANT : utiliser SAVE_KEY comme clé explicite si le store
        // est en mode "out-of-line keys" (sans keyPath)
        const store = tx.objectStore(STORE_STATE);
        if (store.keyPath) {
          // Le store a un keyPath → snap.key doit être présent
          if (!snap.key) snap.key = SAVE_KEY;
          req = store.put(snap);
        } else {
          // Pas de keyPath → on doit fournir la clé en 2e argument
