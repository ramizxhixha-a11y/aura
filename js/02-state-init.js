...
// [S1 · 25/08/2026] Mon moteur funding du 23/08 est RETIRÉ : l'audit S1 a prouvé
// qu'un circuit complet préexistait (applyFundingFees 30s sur les positions +
// applyLeverageBorrowFees sur la dette, intérêts routés en réserve fiscale) — le
// mien aurait DOUBLE-prélevé la dette au premier emprunt. La règle de Rams (funding
// réaliste) est déjà servie par l'existant, mieux intégré.
...
// ── MIGRATION TABLE FISCALE ──────────────────────────────────────
// Les taux d'imposition sont des LOIS, pas des données utilisateur :
// ils doivent toujours refléter la table courante, même si une vieille
// sauvegarde restaure une table périmée. _ensureTaxRegions() réimpose
...
async function fetchLivePrices(force = false) {
  ...
  try {
    ...
    if(updated > 0) {
      ...
      // [PHASE 1 · 12/09/2026] _cgT('liveTrainAgents') RETIRÉ : la fonction (03) est archivée (archive/liveTrainAgents-03-retire-phase1.js) — doublon de l'AT, écrasait le score appris.
    }
...
// La loi belge n'a PAS de seuil chiffré : le caractère spéculatif s'apprécie
// par un FAISCEAU D'INDICES (intention, risque). Le bot s'auto-évalue sur ses
// données réelles via un score composite ; le résultat évolue dans le temps.
// Indices retenus (sources publiques BE 2026) :
//   - recours à l'emprunt (levier)            → indice fort
//   - périodes de détention courtes           → indice
//   - fréquence réelle rapportée aux jours     → indice
//   - disproportion montant investi / capital  → indice
// Retourne { regime, rate, franchise, reg, longTermMonths, reason, specScore, isSpec }
function detectFiscalRegime() {
  ...
  // ── Indice 1 : recours à l'emprunt (levier) — le plus déterminant en droit BE ──
  // ATTENTION : seul l'emprunt RÉELLEMENT ENGAGÉ compte (leverageBorrowed > 0).
  // Régler le curseur levier (S.leverage) sans qu'aucun trade ne dépasse le compte
  // trading n'engage AUCUN emprunt (logique JIT, cf. 09c) → ce n'est pas un recours
  // à l'emprunt au sens fiscal. Le curseur = capacité dispo, pas une dette.
  const usesLeverage = (S.leverageBorrowed || 0) > 0;
  if (usesLeverage) { score += 2; indices.push('levier'); }

  // ── Indice 2 : durée de détention moyenne courte ──
  // Le bot calcule la moyenne réelle des durées de détention (holdMs des trades fermés).
  try {
    const durs = fl.map(e => e.holdMs).filter(d => typeof d === 'number' && d > 0);
    if (durs.length >= 5) {
      const avgMs = durs.reduce((s,d)=>s+d,0) / durs.length;
      const avgMin = avgMs / 60000;
      if (avgMin < 60)       { score += 2; indices.push('détention <1h'); }   // très court terme
      else if (avgMin < 360) { score += 1; indices.push('détention <6h'); }   // court terme
    }
  } catch(e) {}

  // ── Indice 3 : fréquence réelle rapportée aux jours d'activité ──
  // trades ÷ jours écoulés depuis le 1er trade horodaté. Un jour calme compte autant
  // qu'un jour intense : c'est la fréquence soutenue dans le temps qui est un indice.
  try {
    const stamps = fl.map(e => e.ts).filter(t => typeof t === 'number' && t > 0);
    if (stamps.length >= 5) {
      const oldest = Math.min.apply(null, stamps);
      const days = Math.max(1, (Date.now() - oldest) / 86400000);
      const perDay = stamps.length / days;
      if (perDay >= 20)      { score += 2; indices.push('≥20 trades/j'); }
      else if (perDay >= 8)  { score += 1; indices.push('≥8 trades/j'); }
    }
  } catch(e) {}

  // ── Indice 4 : disproportion montant investi vs capital propre ──
  try {
    const capital = (S.ownFundsInjected || 0) + (S.cashAccount || 0);
    const notionals = fl.map(e => e.notional).filter(n => typeof n === 'number' && n > 0);
    if (capital > 0 && notionals.length >= 5) {
      const avgNotional = notionals.reduce((s,n)=>s+n,0) / notionals.length;
      if (avgNotional > capital * 0.5) { score += 1; indices.push('mises disproportionnées'); }
    }
  } catch(e) {}

  // Seuil : score ≥ 2 → spéculatif. Le levier seul (score 2) suffit ; sinon il faut
  // au moins deux indices faibles convergents. Score évolutif, recalculé en continu.
  const isSpec = score >= 2;
  const reason = isSpec
    ? (indices.length ? indices.join(' · ') : 'profil spéculatif')
    : 'gestion normale';
  ...
  return {
    ...
    reg, reason, isSpec, specScore: score, specIndices: indices
  };
}
...
// ════════════════════════════════════════════════════════════════════════
// VALIDATION ANTI-NÉGATIF (au pire stop-loss estimé)
// ════════════════════════════════════════════════════════════════════════
// Principe (décidé avec l'utilisateur) : au pire, le bot ne peut JAMAIS perdre
// plus que son DISPONIBLE RÉEL — c.-à-d. ce qui reste dans le compte trading
// APRÈS avoir mis de côté tout ce qui est dû (frais + intérêts + taxe). Le
// négatif est interdit. Le SL n'est pas figé : le bot l'estime selon la
// volatilité du moment (ATR), avec une marge car un stop peut sauter (gap,
// flash-crash, coupure réseau). On prend donc le pire SL plausible.
//   ok=true  → la perte max plausible tient dans le disponible
//   ok=false → réduire l'exposition (l'appelant ajuste, sinon s'abstient)
// Retourne { ok, worstLoss, costs, available, slPct, totalExposure }.
function validateAntiNegative(baseStake, levTotal, ps) {
  const stake = Math.max(0, baseStake || 0);
  const borrowed = Math.max(0, levTotal || 0);
  const totalExposure = stake + borrowed;

  // ── SL estimé selon la volatilité réelle (pas figé) ──
  // Base : SL configuré (défaut 3%). Le bot le compare à une estimation ATR :
  // si la volatilité actuelle implique un SL plus large, on retient le plus
  // large (prudent). Marge ×1.5 car un stop peut ne pas tenir (gap/crash).
  let slPct = (S.feeConfig && S.feeConfig.stopLossPct) || 3.0;
  try {
    if (ps && Array.isArray(ps.candles) && ps.candles.length >= 15 && ps.price > 0) {
      const candles = ps.candles.slice(-20);
      const ranges = candles.map(c => (c.h && c.l) ? (c.h - c.l) : 0).filter(v => v > 0);
      if (ranges.length >= 10) {
        const avgATR = ranges.reduce((a, b) => a + b, 0) / ranges.length;
        const atrPct = (avgATR / ps.price) * 100;     // volatilité moyenne en %
        // SL plausible = ~2× l'ATR (un mouvement adverse typique), borné bas par
        // le SL configuré. Marge de sécurité ×1.5 pour le risque de saut de stop.
        const volSL = atrPct * 2;
        slPct = Math.max(slPct, volSL) * 1.5;
      }
    } else {
      // Sans données fiables : marge prudente sur le SL configuré
      slPct = slPct * 1.5;
    }
  } catch (e) { slPct = slPct * 1.5; }
  // Borne haute raisonnable : on ne provisionne pas au-delà de 25% (au-delà,
  // c'est l'exposition elle-même qu'il faut réduire, pas la provision).
  slPct = Math.min(25, slPct);

  // Perte au pire cas, sur l'exposition TOTALE (le levier amplifie).
  const worstLoss = totalExposure * (slPct / 100);

  // ── Coûts dus à mettre de côté (frais + slippage + intérêts + taxe) ──
  let costs = 0;
  try {
    if (typeof estimateTradeReserve === 'function') {
      costs += estimateTradeReserve(totalExposure, borrowed).total || 0;
    }
  } catch (e) {}
  try {
    const reg = (S.taxConfig && S.taxConfig.regions && S.taxConfig.regions[S.taxConfig.region]) || {};
    const rateSpec = (typeof reg.rateSpec === 'number') ? reg.rateSpec : (reg.rate || 0);
    // Si emprunt → régime spéculatif : on provisionne la taxe sur un gain
    // hypothétique (symétrie prudente avec la perte au SL), au taux spéculatif.
    if (borrowed > 0) costs += worstLoss * rateSpec;
  } catch (e) {}

  // ── Disponible réel risquable = compte trading − tout ce qui est dû ──
  const available = (S.tradingAccount || 0) - costs;

  // Contrainte absolue : la perte max plausible ne dépasse pas le disponible.
  return {
    ok: worstLoss <= available,
    worstLoss, costs, available, slPct, totalExposure
  };
}
...
function computePortfolioTotal(){
  ...
  const baseUSD = ((typeof _computePortfolio==='function') ? _computePortfolio() : ((S.cashAccount||0)+(S.tradingAccount||0))) + (S.fiscalReserveAccount || 0) + (S.antiNegTaxPart || 0) + ((S.fees&&S.fees.feeReserveAccount)||0);   // [23/08 · Rams] + provisions taxes + réserve frais (le funding y est déjà provisionné : intérêts→fiscale, funding positions→frais — S1 25/08)
...
function changeLeverage(delta) {
  ...
  if(typeof _renderWalletCards === 'function') { try { _renderWalletCards(); } catch(e){} }
  else if(typeof window !== 'undefined' && typeof window._renderWalletCards === 'function') { try { window._renderWalletCards(); } catch(e){} }
